-- Critical security fixes for database functions and audit logging

-- 1. Fix function security by setting proper search_path
...
-- 2. Add audit triggers to sensitive tables for monitoring access
DROP TRIGGER IF EXISTS audit_assessment_contacts ON public.assessment_contacts;
CREATE TRIGGER audit_assessment_contacts
  AFTER SELECT OR UPDATE OR DELETE ON public.assessment_contacts
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();

DROP TRIGGER IF EXISTS audit_form_submissions ON public.form_submissions;  
CREATE TRIGGER audit_form_submissions
  AFTER SELECT OR UPDATE OR DELETE ON public.form_submissions
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();

DROP TRIGGER IF EXISTS audit_assessment_sessions ON public.assessment_sessions;
CREATE TRIGGER audit_assessment_sessions
  AFTER SELECT ON public.assessment_sessions
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();

DROP TRIGGER IF EXISTS audit_admin_profiles ON public.admin_profiles;
CREATE TRIGGER audit_admin_profiles
  AFTER SELECT OR UPDATE OR DELETE ON public.admin_profiles
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();