...
-- Phase 1: Immediate database security hardening
...
-- 4. SECURE DATA ACCESS LOGGING

-- Enhanced trigger for sensitive data access logging
CREATE OR REPLACE FUNCTION public.log_enhanced_sensitive_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  INSERT INTO public.analytics_events (
    ...
    'sensitive_data_access_enhanced',
    jsonb_build_object(
...
-- Apply enhanced logging to critical tables
DROP TRIGGER IF EXISTS assessment_sessions_access_log ON public.assessment_sessions;
CREATE TRIGGER assessment_sessions_access_log
  AFTER SELECT ON public.assessment_sessions
  FOR EACH ROW EXECUTE FUNCTION public.log_enhanced_sensitive_access();

DROP TRIGGER IF EXISTS admin_profiles_access_log ON public.admin_profiles;
CREATE TRIGGER admin_profiles_access_log
  AFTER SELECT ON public.admin_profiles
  FOR EACH ROW EXECUTE FUNCTION public.log_enhanced_sensitive_access();
