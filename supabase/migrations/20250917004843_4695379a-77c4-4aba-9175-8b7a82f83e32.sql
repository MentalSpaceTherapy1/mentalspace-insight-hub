...
-- 6. Enhance chat_sessions security
...
-- 7. Create audit logging function for sensitive data access
...
CREATE TRIGGER log_assessment_sessions_access
  AFTER SELECT OR INSERT OR UPDATE ON public.assessment_sessions
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();

CREATE TRIGGER log_assessment_contacts_access  
  AFTER SELECT OR INSERT OR UPDATE ON public.assessment_contacts
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();

CREATE TRIGGER log_form_submissions_access
  AFTER SELECT OR INSERT OR UPDATE ON public.form_submissions  
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_data_access();